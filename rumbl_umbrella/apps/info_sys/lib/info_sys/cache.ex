defmodule InfoSys.Cache do
  ...
  def put(name \\ @module_name, key, value) do
    true = :ets.insert(tab_name(name), {key, value})

    :ok
  end

  def fetch(name \\ @module_name, key) do
  ...
  def start_link(opts) do
    opts = Keyword.put_new(opts, :name, @module_name)
    GenServer.start_link(@module_name, opts, name: opts[:name])
  end
